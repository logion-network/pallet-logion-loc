...
use crate::{File, LegalOfficerCase, LocLink, LocType, MetadataItem, CollectionItem, CollectionItemFile, mock::*};
use crate::Error;
...
fn it_fails_adding_item_to_open_collection_loc() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(LogionLoc::add_collection_item(Origin::signed(LOC_OWNER1), LOC_ID, collection_item_id, collection_item_description, vec![]), Error::<Test>::WrongCollectionLoc);
	});
}
...
fn it_adds_item_to_closed_collection_loc() {
	new_test_ext().execute_with(|| {
		...
		let collection_item_id = BlakeTwo256::hash_of(&"item-id".as_bytes().to_vec());
		let collection_item_description = "item-description".as_bytes().to_vec();
		assert_ok!(LogionLoc::add_collection_item(Origin::signed(LOC_REQUESTER_ID), LOC_ID, collection_item_id, collection_item_description.clone(), vec![]));
...
fn it_fails_adding_item_if_duplicate_key() {
	new_test_ext().execute_with(|| {
		...
		let collection_item_id = BlakeTwo256::hash_of(&"item-id".as_bytes().to_vec());
		let collection_item_description = "item-description".as_bytes().to_vec();
		assert_ok!(LogionLoc::add_collection_item(Origin::signed(LOC_REQUESTER_ID), LOC_ID, collection_item_id.clone(), collection_item_description.clone(), vec![]));
		assert_err!(LogionLoc::add_collection_item(Origin::signed(LOC_REQUESTER_ID), LOC_ID, collection_item_id, collection_item_description, vec![]), Error::<Test>::CollectionItemAlreadyExists);
	});
}
...
fn it_fails_adding_item_if_size_limit_reached() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(LogionLoc::add_collection_item(Origin::signed(LOC_REQUESTER_ID), LOC_ID, collection_item_id2, collection_item_description, vec![]), Error::<Test>::CollectionLimitsReached);
	});
}
...
fn it_fails_adding_item_if_block_limit_reached() {
	...
	new_test_ext_at_block(current_block).execute_with(|| {
		...
		let collection_item_id = BlakeTwo256::hash_of(&"item-id".as_bytes().to_vec());
		let collection_item_description = "item-description".as_bytes().to_vec();
		assert_err!(LogionLoc::add_collection_item(Origin::signed(LOC_REQUESTER_ID), LOC_ID, collection_item_id, collection_item_description, vec![]), Error::<Test>::CollectionLimitsReached);
	});
}
...
fn it_fails_adding_item_if_collection_void() {
	new_test_ext().execute_with(|| {
		...
		let collection_item_id = BlakeTwo256::hash_of(&"item-id".as_bytes().to_vec());
		let collection_item_description = "item-description".as_bytes().to_vec();
		assert_err!(LogionLoc::add_collection_item(Origin::signed(LOC_REQUESTER_ID), LOC_ID, collection_item_id, collection_item_description, vec![]), Error::<Test>::WrongCollectionLoc);
	});
}
...
fn it_fails_adding_item_if_files_attached_but_upload_not_enabled() {
	new_test_ext().execute_with(|| {
		...
		assert_err!(LogionLoc::add_collection_item(Origin::signed(LOC_REQUESTER_ID), LOC_ID, collection_item_id, collection_item_description, collection_item_files), Error::<Test>::CannotUpload);
	});
}
